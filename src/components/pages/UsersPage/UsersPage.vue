  ...
  <div class="app">
    ...
    <main class="main">
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Member since</th><th>Actions</th></tr>
          </thead>
          <tbody>
            <tr v-for="user in filteredUsers" :key="user.userId">
              <td style="font-weight:600">{{ fullName(user) }}</td>
              <td style="color:var(--ink-mute)">{{ user.email }}</td>
              <td><span :class="user.role === 'EMPLOYEE' ? 'badge badge-purple' : 'badge badge-blue'">{{ user.role }}</span></td>
