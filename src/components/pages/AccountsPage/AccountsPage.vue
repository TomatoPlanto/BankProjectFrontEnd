<template>
  <div class="app">
    ...
    <main class="main">
      ...
      <div class="pager" v-if="accountStore.totalPages > 1">
        <button class="btn btn-ghost btn-sm" :disabled="accountStore.page === 0"
                @click="accountStore.fetchAllAccounts(accountStore.page - 1)">‹ Prev</button>
        <span class="pager-info">
          Page {{ accountStore.page + 1 }} of {{ accountStore.totalPages }}
          · {{ accountStore.totalElements }} accounts
        </span>
        <button class="btn btn-ghost btn-sm" :disabled="accountStore.page >= accountStore.totalPages - 1"
                @click="accountStore.fetchAllAccounts(accountStore.page + 1)">Next ›</button>
      </div>
    </main>
...
onMounted(() => accountStore.fetchAllAccounts(0))
