  ...
  <div class="app">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="account in accountStore.accounts" :key="account.accountId">
              ...
              <td><span :class="account.accountType === 'CHECKING' ? 'badge badge-blue' : 'badge badge-purple'">{{ account.accountType }}</span></td>
              ...
              <td><span :class="account.status === 'ACTIVE' ? 'badge badge-green' : 'badge badge-red'">{{ account.status }}</span></td>
              <td>
    ...
    <div v-if="selectedAccount" class="modal-overlay" @click.self="selectedAccount = null">
      <div class="modal">
        ...
        <div style="font-family:monospace; font-size:12px; color:var(--ink-mute); margin-bottom:1.5rem">{{ selectedAccount.iban }}</div>
        <div class="field"><label class="label">Daily limit</label><input v-model.number="limitsForm.dailyLimit" type="number" class="input" /></div>
        <div class="field"><label class="label">Transfer limit</label><input v-model.number="limitsForm.transferLimit" type="number" class="input" /></div>
        <div class="field"><label class="label">Absolute minimum</label><input v-model.number="limitsForm.absoluteMinimum" type="number" class="input" /></div>
...
function openLimits(account) {
  ...
  limitsForm.value = { dailyLimit: account.dailyLimit, transferLimit: account.transferLimit, absoluteMinimum: account.absoluteMinimum }
}
